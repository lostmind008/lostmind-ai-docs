...
1. Run pnpm run update:check to see what's outdated
2. Identify which updates are safe (patch/minor)
3. Update safe ones first
4. Test everything still works
5. Create checkpoint after successful updates
```

### Security Fix Request
```
Please fix security vulnerabilities:
1. Run pnpm audit to check issues
2. Explain what the vulnerabilities are
3. Run pnpm audit fix
4. Test the application
5. If audit fix doesn't work, update packages manually
```

### Major Version Update
```
I want to update [package] to the next major version:
1. Check the migration guide
2. Explain breaking changes
...
- [Security Best Practices](https://snyk.io/blog/nodejs-security-best-practices/)

## 🆘 Getting Help

If you're unsure about an update:

1. Ask Claude Code: "Is it safe to update [package]?"
2. Check the package's GitHub for breaking changes
3. Create a backup branch first
4. Test in development before production
5. Ask in the PR comments if using Dependabot

Remember: **It's always safer to be cautious with updates!**